...
void CESP::StorePlayers(CTFPlayer* pLocal)
{
	...
	for (auto pEntity : H::Entities.GetGroup(EGroupType::PLAYERS_ALL))
	{
		...
		if (Vars::ESP::Player.Value & Vars::ESP::PlayerEnum::ClassText)
			tCache.m_vText.emplace_back(ESPTextEnum::Right, GetPlayerClass(iClassNum), Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
		...
		if (Vars::ESP::Player.Value & Vars::ESP::PlayerEnum::WeaponText && pWeapon)
			tCache.m_vText.push_back({ ESPTextEnum::Bottom, pWeapon->GetWeaponName(), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value });
		...
		if (Vars::Debug::Info.Value && !pPlayer->IsDormant() && pPlayer->entindex() != I::EngineClient->GetLocalPlayer())
		{
			...
			tCache.m_vText.emplace_back(ESPTextEnum::Right, std::format("LAG {}, {}", iAverage, iCurrent), Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
		}

		{
			if (Vars::ESP::Player.Value & Vars::ESP::PlayerEnum::LagCompensation && !pPlayer->IsDormant() && pPlayer != pLocal)
			{
				if (H::Entities.GetLagCompensation(pPlayer->entindex()))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "LC", Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
			}
			...
			if (Vars::ESP::Player.Value & Vars::ESP::PlayerEnum::Buffs)
			{
				...
				if (bCrits)
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "CRITS", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				else if (bMiniCrits)
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "MINI-CRITS", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				...
				if (pPlayer->InCond(TF_COND_INVULNERABLE) ||
					...
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "UBER", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				else if (pPlayer->InCond(TF_COND_PHASE))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "BONK", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);

				/* vaccinator effects */
				if (pPlayer->InCond(TF_COND_MEDIGUN_UBER_BULLET_RESIST) || pPlayer->InCond(TF_COND_BULLET_IMMUNE))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "BULLET+", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				...
				if (pPlayer->InCond(TF_COND_MEDIGUN_UBER_BLAST_RESIST) || pPlayer->InCond(TF_COND_BLAST_IMMUNE))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "BLAST+", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				...
				if (pPlayer->InCond(TF_COND_MEDIGUN_UBER_FIRE_RESIST) || pPlayer->InCond(TF_COND_FIRE_IMMUNE))
					...
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "FIRE", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);

				if (pPlayer->InCond(TF_COND_OFFENSEBUFF))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "BANNER", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				if (pPlayer->InCond(TF_COND_DEFENSEBUFF))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "BATALLIONS", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				if (pPlayer->InCond(TF_COND_REGENONDAMAGEBUFF))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "CONCH", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				...
				if (pPlayer->InCond(TF_COND_RUNE_STRENGTH))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "STRENGTH", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				if (pPlayer->InCond(TF_COND_RUNE_HASTE))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "HASTE", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				if (pPlayer->InCond(TF_COND_RUNE_REGEN))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "REGEN", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				if (pPlayer->InCond(TF_COND_RUNE_RESIST))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "RESISTANCE", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				if (pPlayer->InCond(TF_COND_RUNE_VAMPIRE))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "VAMPIRE", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				if (pPlayer->InCond(TF_COND_RUNE_REFLECT))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "REFLECT", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				if (pPlayer->InCond(TF_COND_RUNE_PRECISION))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "PRECISION", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				if (pPlayer->InCond(TF_COND_RUNE_AGILITY))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "AGILITY", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				if (pPlayer->InCond(TF_COND_RUNE_KNOCKOUT))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "KNOCKOUT", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				if (pPlayer->InCond(TF_COND_RUNE_KING))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "KING", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				if (pPlayer->InCond(TF_COND_RUNE_PLAGUE))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "PLAGUE", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				if (pPlayer->InCond(TF_COND_RUNE_SUPERNOVA))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "SUPERNOVA", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				if (pPlayer->InCond(TF_COND_POWERUPMODE_DOMINANT))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "DOMINANT", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
			}
			...
			if (Vars::ESP::Player.Value & Vars::ESP::PlayerEnum::Debuffs)
			{
				if (pPlayer->InCond(TF_COND_MARKEDFORDEATH) || pPlayer->InCond(TF_COND_MARKEDFORDEATH_SILENT))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "MARKED", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);

				if (pPlayer->InCond(TF_COND_URINE))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "JARATE", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);

				if (pPlayer->InCond(TF_COND_MAD_MILK))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "MILK", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);

				if (pPlayer->InCond(TF_COND_STUNNED))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "STUN", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);

				if (pPlayer->InCond(TF_COND_BURNING))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "BURN", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);

				if (pPlayer->InCond(TF_COND_BLEEDING))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "BLEED", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
			}
			...
			if (Vars::ESP::Player.Value & Vars::ESP::PlayerEnum::Misc)
			{
				if (Vars::Visuals::Removals::Taunts.Value && pPlayer->InCond(TF_COND_TAUNTING))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "TAUNT", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);

				if (pPlayer->m_bFeignDeathReady())
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "FEIGN", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				...
				if (pPlayer->InCond(TF_COND_AIMING) || pPlayer->InCond(TF_COND_ZOOMED))
				{
					...
					case TF_WEAPON_MINIGUN:
						tCache.m_vText.emplace_back(ESPTextEnum::Right, "REV", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
						break;
					...
					case TF_WEAPON_SNIPERRIFLE_DECAP:
					{
						...
						{
							tCache.m_vText.emplace_back(ESPTextEnum::Right, std::format("CHARGING {:.0f}%%", Math::RemapVal(pWeapon->As<CTFSniperRifle>()->m_flChargedDamage(), 0.f, 150.f, 0.f, 100.f)), Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
							break;
						}
						else
						{
							...
							if (pPlayerDot)
							{
								...
								tCache.m_vText.emplace_back(ESPTextEnum::Right, std::format("CHARGING {:.0f}%%", Math::RemapVal(TICKS_TO_TIME(I::ClientState->m_ClockDriftMgr.m_nServerTick) - pPlayerDot->m_flChargeStartTime() - 0.3f, 0.f, flChargeTime, 0.f, 100.f)), Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
								break;
							}
						}
						tCache.m_vText.emplace_back(ESPTextEnum::Right, "CHARGING", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
						break;
					}
					case TF_WEAPON_COMPOUND_BOW:
						if (iIndex == I::EngineClient->GetLocalPlayer())
						{
							tCache.m_vText.emplace_back(ESPTextEnum::Right, std::format("CHARGING {:.0f}%%", Math::RemapVal(TICKS_TO_TIME(I::ClientState->m_ClockDriftMgr.m_nServerTick) - pWeapon->As<CTFPipebombLauncher>()->m_flChargeBeginTime(), 0.f, 1.f, 0.f, 100.f)), Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
							break;
						}
						tCache.m_vText.emplace_back(ESPTextEnum::Right, "CHARGING", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
						break;
				...
				if (pPlayer->InCond(TF_COND_SHIELD_CHARGE))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "CHARGE", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				...
				if (pPlayer->InCond(TF_COND_STEALTHED) || pPlayer->InCond(TF_COND_STEALTHED_BLINK) || pPlayer->InCond(TF_COND_STEALTHED_USER_BUFF) || pPlayer->InCond(TF_COND_STEALTHED_USER_BUFF_FADING))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, std::format("INVIS {:.0f}%%", pPlayer->GetInvisPercentage()), Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);

				if (pPlayer->InCond(TF_COND_DISGUISING) || pPlayer->InCond(TF_COND_DISGUISED))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "DISGUISE", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
			}
			if (Vars::ESP::Player.Value & Vars::ESP::PlayerEnum::Distance && pPlayer != pLocal)
			{
				Vec3 vDelta = pPlayer->m_vecOrigin() - pLocal->m_vecOrigin();
				tCache.m_vText.emplace_back(ESPTextEnum::Bottom, std::format("[{:.0f}M]", vDelta.Length2D() / 41), Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
			}
		}
	}
}
...
void CESP::StoreBuildings(CTFPlayer* pLocal)
{
	...
	for (auto pEntity : H::Entities.GetGroup(EGroupType::BUILDINGS_ALL))
	{
		...
		if (Vars::ESP::Building.Value & Vars::ESP::BuildingEnum::HealthText)
			tCache.m_vText.emplace_back(ESPTextEnum::Health, std::format("{}", flHealth), Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
		...
		if (Vars::ESP::Building.Value & Vars::ESP::BuildingEnum::Level && !bIsMini)
			tCache.m_vText.emplace_back(ESPTextEnum::Right, std::format("{} / {}", pBuilding->m_iUpgradeLevel(), bIsMini ? 1 : 3), Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
		...
		if (Vars::ESP::Building.Value & Vars::ESP::BuildingEnum::Flags)
		{
			...
			if (flConstructed < 1.f)
				tCache.m_vText.emplace_back(ESPTextEnum::Right, std::format("{:.0f}%%", flConstructed * 100.f), Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
			...
			if (pBuilding->IsSentrygun() && !pBuilding->m_bBuilding())
			{
				...
				if (!iShells)
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "NO AMMO", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
				if (!bIsMini && !iRockets)
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "NO ROCKETS", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
			}
		}
	}
}
		...
		if (Vars::ESP::Projectile.Value & Vars::ESP::ProjectileEnum::Flags)
		{
			...
			case ETFClassID::CTFProjectile_ThrowableRepel:
				...
				if (pEntity->As<CTFWeaponBaseGrenadeProj>()->m_iDeflected() && (pEntity->GetClassID() != ETFClassID::CTFGrenadePipebombProjectile || !pEntity->GetAbsVelocity().IsZero()))
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "DEFLECTED", Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
				break;
			case ETFClassID::CTFProjectile_Arrow:
			case ETFClassID::CTFProjectile_GrapplingHook:
			case ETFClassID::CTFProjectile_HealingBolt:
	...
	for (auto pEntity : H::Entities.GetGroup(EGroupType::WORLD_OBJECTIVE))
	{
		...
		case ETFClassID::CCaptureFlag:
		{
			...
			if (Vars::ESP::Objective.Value & Vars::ESP::ObjectiveEnum::Name)
				tCache.m_vText.emplace_back(ESPTextEnum::Top, "INTEL", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
			...
			if (Vars::ESP::Objective.Value & Vars::ESP::ObjectiveEnum::Flags)
			{
				...
				case TF_FLAGINFO_HOME:
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "HOME", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
					break;
				case TF_FLAGINFO_DROPPED:
					tCache.m_vText.emplace_back(ESPTextEnum::Right, "DROPPED", Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
					break;
				default:
			...
			if (Vars::ESP::Objective.Value & Vars::ESP::ObjectiveEnum::IntelReturnTime && pIntel->m_nFlagStatus() == TF_FLAGINFO_DROPPED)
			{
				...
				tCache.m_vText.emplace_back(ESPTextEnum::Right, std::format("RETURN {:.1f}s", pIntel->m_flResetTime() - TICKS_TO_TIME(I::ClientState->m_ClockDriftMgr.m_nServerTick)).c_str(), Vars::Menu::Theme::Conditions.Value, Vars::Menu::Theme::Background.Value);
			}
...
void CESP::DrawPlayers()
{
	...
	const auto& fFontFlag = H::Fonts.GetFont(FONT_ESP_CONDS);
	...
	for (auto& [pEntity, tCache] : m_mPlayerCache)
	{
		...
		{
			const bool use_custom = Vars::ESP::CustomHPColor.Value;
			if (tCache.m_flHealth > 1.f)
			{

				Color_t base_color = use_custom ? Vars::ESP::HPColor.Value : Vars::Colors::IndicatorGood.Value;
				H::Draw.FillRectPercent(x - H::Draw.Scale(6), y, H::Draw.Scale(2, Scale_Round), h, 1.f, base_color, { 0, 0, 0, 255 }, ALIGN_BOTTOM, true);

				Color_t overheal_color = use_custom ? Vars::ESP::HPColor.Value : Vars::Colors::IndicatorMisc.Value;
				H::Draw.FillRectPercent(x - H::Draw.Scale(6), y, H::Draw.Scale(2, Scale_Round), h, tCache.m_flHealth - 1.f, overheal_color, { 0, 0, 0, 0 }, ALIGN_BOTTOM, true);
			}
			else
			{
				Color_t cColor = use_custom ? Vars::ESP::HPColor.Value
					: Vars::Colors::IndicatorBad.Value.Lerp(Vars::Colors::IndicatorGood.Value, tCache.m_flHealth);
		...
		for (auto& [iMode, sText, tColor, tOutline] : tCache.m_vText)
		{
			...
			case ESPTextEnum::Bottom:
				H::Draw.StringOutlined(fFontFlag, m, b + bOffset, tColor, tOutline, ALIGN_TOP, sText.c_str());
				bOffset += nTall;
				break;
			case ESPTextEnum::Right:
				H::Draw.StringOutlined(fFontFlag, r, t + iVerticalOffset + rOffset, tColor, tOutline, ALIGN_TOPLEFT, sText.c_str());
				rOffset += nTall;
				break;
...
void CESP::DrawBuildings()
{
	...
	for (auto& [pEntity, tCache] : m_mBuildingCache)
	{
		...
		if (tCache.m_bHealthBar)
		{
			Color_t cColor = Vars::ESP::CustomHPColor.Value	? Vars::ESP::HPColor.Value : Vars::Colors::IndicatorBad.Value.Lerp(Vars::Colors::IndicatorGood.Value, tCache.m_flHealth);
			H::Draw.FillRectPercent(x - H::Draw.Scale(6), y, H::Draw.Scale(2, Scale_Round), h, tCache.m_flHealth, cColor, { 0, 0, 0, 255 }, ALIGN_BOTTOM, true);
			lOffset += H::Draw.Scale(6);
		}
