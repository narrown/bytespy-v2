...
{
	if (!(Vars::Menu::Indicators.Value & Vars::Menu::IndicatorsEnum::Spotify) || !pLocal->IsAlive())
		return;
	...
	for (HWND hwnd = GetTopWindow(nullptr); hwnd; hwnd = GetWindow(hwnd, GW_HWNDNEXT))
	{
		...
		if (!GetModuleFileNameExW(process, nullptr, filename, MAX_PATH)) {
			CloseHandle(process);
			continue;
		}
		CloseHandle(process);

		if (std::wstring(filename).find(L"Spotify.exe") == std::wstring::npos)
			continue;

		wchar_t window_title[300]{};
		if (!GetWindowTextW(hwnd, window_title, 300))
			continue;

		const DragBox_t spotifyPos = Vars::Menu::SpotifyDisplay.Value;
		const auto& fFont = H::Fonts.GetFont(FONT_INDICATORS);

		std::wstring wide_title(window_title);
		const bool is_playing = wide_title.find(L"-") != std::wstring::npos;
		const bool is_ad = wide_title.find(L"Advertisement") != std::wstring::npos;
		const std::wstring display_text = is_playing ? wide_title : L"Spotify: Not Playing";

		int textW = 0, textH = 0, statusW = 0, statusH = 0;
		I::MatSystemSurface->GetTextSize(fFont.m_dwFont, display_text.c_str(), textW, textH);

		std::wstring statusText = is_ad ? L"AD" : L"";
		if (is_ad)
			I::MatSystemSurface->GetTextSize(fFont.m_dwFont, statusText.c_str(), statusW, statusH);

		const int padding = H::Draw.Scale(4);
		const int spacing = H::Draw.Scale(4);
		const int barHeight = H::Draw.Scale(2);
		const int totalWidth = textW + (is_ad ? (statusW + spacing) : 0) + 2 * padding;
		const int totalHeight = padding + std::max(textH, statusH) + spacing + barHeight + padding;

		const int posX = spotifyPos.x - totalWidth / 2;
		const int posY = spotifyPos.y;
		if (!Vars::Menu::RemoveBackground.Value)
		{
			H::Draw.FillRoundRect(posX, posY, totalWidth, totalHeight, H::Draw.Scale(4), { 0, 0, 0, 150 });
		}
		H::Draw.String(fFont, posX + padding, posY + padding, Vars::Menu::Theme::Active.Value, ALIGN_TOPLEFT, display_text.c_str());

		if (is_ad)
		{
			H::Draw.String(
				fFont,
				posX + totalWidth - padding,
				posY + padding,
		...
		const int barY = posY + padding + std::max(textH, statusH) + spacing;
		const int barWidth = totalWidth - 2 * padding;
		H::Draw.FillRect(posX + padding, barY, barWidth, barHeight, Vars::Menu::Theme::Accent.Value);
	}
}
...
void CVisuals::DrawWatermark()
{
	...
	const int height = H::Draw.Scale(25);
	const int lineHeight = H::Draw.Scale(2);
	const int padding = H::Draw.Scale(4);
	const int spacing = H::Draw.Scale(4);

	std::string part1 = "byte";
	std::string part2 = "spy";
	std::string part3 = "v2.5";
	std::string tag;

	PlayerInfo_t info{};
	if (I::EngineClient->GetPlayerInfo(I::EngineClient->GetLocalPlayer(), &info))
	{
		const uint64 steamID64 = 0x110000100000000ULL | info.friendsID;

		if (steamID64 == 76561199130658908ULL) tag = "| narrow"; // main
		if (steamID64 == 76561199682314952ULL) tag = "| narrow"; // alt
		if (steamID64 == 76561199193356642ULL) tag = "| troy hickle"; // billy
		if (steamID64 == 76561199574795060ULL) tag = "| grizz"; // main
		if (steamID64 == 76561198899885569ULL) tag = "| grizz"; // alt 1
		if (steamID64 == 76561199434000498ULL) tag = "| grizz"; // alt 2
		if (steamID64 == 76561199073142280ULL) tag = "| ascendedsion"; // main
		if (steamID64 == 76561199746755990ULL) tag = "| ascendedsion"; // alt
		if (steamID64 == 76561199859378169ULL) tag = "| ascendedsion"; // alt 2
	}

	std::wstring wPart1(part1.begin(), part1.end());
	std::wstring wPart2(part2.begin(), part2.end());
	std::wstring wPart3(part3.begin(), part3.end());
	std::wstring wTag(tag.begin(), tag.end());

	int w1 = 0, h1 = 0, w2 = 0, h2 = 0, w3 = 0, h3 = 0, wTagVal = 0, hTag = 0;

	I::MatSystemSurface->GetTextSize(font.m_dwFont, wPart1.c_str(), w1, h1);
	I::MatSystemSurface->GetTextSize(font.m_dwFont, wPart2.c_str(), w2, h2);
	I::MatSystemSurface->GetTextSize(font.m_dwFont, wPart3.c_str(), w3, h3);
	I::MatSystemSurface->GetTextSize(font.m_dwFont, wTag.c_str(), wTagVal, hTag);

	const int totalWidth = w1 + w2 + spacing + w3 + (tag.empty() ? 0 : spacing + wTagVal) + padding * 2;
	const int screenWidth = H::Draw.m_nScreenW;
	const int posX = screenWidth - totalWidth - H::Draw.Scale(10);
	const int posY = H::Draw.Scale(10);

	if (!Vars::Menu::RemoveBackground.Value)
	{
		H::Draw.FillRoundRect(posX, posY, totalWidth, height, H::Draw.Scale(4), { 0, 0, 0, 150 });
	}

	int cursorX = posX + padding;

	H::Draw.StringOutlined(font, cursorX, posY + padding,
		Vars::Menu::Theme::Active.Value, { 0, 0, 0, 150 }, ALIGN_TOPLEFT, part1.c_str());

	cursorX += w1;
	H::Draw.StringOutlined(font, cursorX, posY + padding,
		Vars::Menu::Theme::Accent.Value, { 0, 0, 0, 150 }, ALIGN_TOPLEFT, part2.c_str());

	cursorX += w2 + spacing;
	H::Draw.StringOutlined(font, cursorX, posY + padding,
		Vars::Menu::Theme::Accent.Value, { 0, 0, 0, 150 }, ALIGN_TOPLEFT, part3.c_str());

	if (!tag.empty())
	{
		cursorX += w3 + spacing;
		H::Draw.StringOutlined(font, cursorX, posY + padding,
			Vars::Menu::Theme::Active.Value, { 0, 0, 0, 150 }, ALIGN_TOPLEFT, tag.c_str());
	}

	const int lineY = posY + height - lineHeight - padding;
	H::Draw.FillRect(posX + padding, lineY, totalWidth - padding * 2, lineHeight,
...
void CVisuals::DrawPing(CTFPlayer* pLocal)
{
	...
	// update fake latency every 0.5s
	...
	float flFakeLerp = F::Backtrack.GetFakeInterp() > G::Lerp
		? F::Backtrack.GetFakeInterp()
		: 0.f;

	const float flFake = std::min(flFakeLatency + flFakeLerp, F::Backtrack.m_flMaxUnlag) * 1000.f;
	const float flReal = std::max(
		pNetChan->GetLatency(FLOW_INCOMING)
		+ pNetChan->GetLatency(FLOW_OUTGOING)
		- flFakeLatency,
		0.f
	) * 1000.f;

	const int iScoreboard = pResource->GetPing(pLocal->entindex());

	const DragBox_t dtPos = Vars::Menu::PingDisplay.Value;
	const auto& fFont = H::Fonts.GetFont(FONT_INDICATORS);
	...
	// Conditionally draw the background
	if (!Vars::Menu::RemoveBackground.Value)
	{
	...
	H::Draw.StringOutlined(
		...
		{ 0, 0, 0, 150 },
		ALIGN_TOPLEFT,
		std::format("ping {:.0f}ms", flReal).c_str()
	);
	H::Draw.StringOutlined(
		fFont,
		...
		{ 0, 0, 0, 150 },
		ALIGN_TOPRIGHT,
		std::format("{:.0f}ms", flFake).c_str()
	);

	// compute raw bar ratio
	const float flMaxLatency = 200.f;
	float flTargetRatio = std::min(flReal / flMaxLatency, 1.f);

	// —— smoothing setup ——
	static float flDisplayedPingRatio = 0.0f;
	constexpr float SMOOTH_SPEED = 15.0f;
	float delta = I::GlobalVars->frametime * SMOOTH_SPEED;
	delta = std::clamp(delta, 0.0f, 1.0f);
	flDisplayedPingRatio += (flTargetRatio - flDisplayedPingRatio) * delta;
	// ——

	// draw interpolated bar
	const int iBarY = iPosY + iHeight - iLineHeight - iPadding;
	const int iBarWidth = static_cast<int>((iWidth - iPadding * 2) * flDisplayedPingRatio);

	H::Draw.FillRect(iPosX + iPadding, iBarY, iWidth - iPadding * 2, iLineHeight, { 45, 45, 45, 200 });
	if (iBarWidth > 0)
	{
		H::Draw.FillRect(
			iPosX + iPadding,
	...
	// optional scoreboard ping in debug
	if (Vars::Debug::Info.Value)
	{
		H::Draw.StringOutlined(
			fFont,
			iPosX + iWidth / 2,
			iBarY + iLineHeight + iPadding,
			Vars::Menu::Theme::Active.Value,
			{ 0, 0, 0, 150 },
			ALIGN_TOP,
			std::format("score: {}ms", iScoreboard).c_str()
		);
	}
}
...
void CVisuals::FreecamCM(CUserCmd* pCmd) {
	if (Vars::Misc::Game::Freecam.Value) {
			pCmd->forwardmove = pCmd->sidemove = 0.0f;
	}
	G::SilentAngles = true;
}

void CVisuals::Freecam(CTFPlayer* pLocal, CViewSetup* pView) {
	if (!Vars::Misc::Game::Freecam.Value || !pView)
		return;

	if (originBackup.IsZero()) {
	...
	Math::AngleVectors(currentAngles, &vForward, &vRight, &vUp);

	float speed = Vars::Misc::Game::FreecamSpeed.Value;
	Vec3 newOrigin = originBackup;

	if (GetAsyncKeyState('W') & 0x8000) newOrigin += vForward * speed;
	if (GetAsyncKeyState('S') & 0x8000) newOrigin -= vForward * speed;
	if (GetAsyncKeyState('A') & 0x8000) newOrigin -= vRight * speed;
	if (GetAsyncKeyState('D') & 0x8000) newOrigin += vRight * speed;
	if (GetAsyncKeyState(VK_SPACE) & 0x8000)   newOrigin += vUp * speed;
	if (GetAsyncKeyState(VK_CONTROL) & 0x8000) newOrigin -= vUp * speed;

	pView->origin = newOrigin;
	originBackup = newOrigin;
}
...
void CVisuals::DrawDebugInfo(CTFPlayer* pLocal)
{
	if (Vars::Debug::Info.Value)
	{
		...
		H::Draw.StringOutlined(fFont, x, y += nTall, Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value, ALIGN_TOPLEFT,
			std::format("DT Wait = {}", F::Ticks.m_iWait ? "true" : "false").c_str());
...
void CVisuals::DrawPath(std::deque<Vec3>& Line, Color_t Color, int iStyle, bool bZBuffer, float flTime)
{
	...
	for (size_t i = 1; i < Line.size(); i++)
	{
		...
		case Vars::Visuals::Simulation::StyleEnum::Outlined:
		{
			Color_t black = { 0, 0, 0, Color.a };
			const Vec3 prevPos = Line[i - 1];
			const Vec3 currentPos = Line[i];
			const Vec3 delta = currentPos - prevPos;
			Vec3 perp = Vec3(-delta.y, delta.x, 0.0f).Normalized();

			const float thickness = 1.0f;
			for (float t = -thickness; t <= thickness; t += 0.25f)
			{
				RenderLine(prevPos + perp * t, currentPos + perp * t, black, bZBuffer);
			}

			RenderLine(prevPos, currentPos, Color, bZBuffer);
			break;
		}
