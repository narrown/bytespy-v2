...
bool CRadar::GetDrawPosition(CTFPlayer * pLocal, CBaseEntity * pEntity, int& x, int& y, int& z)
{
	...
	// Increased icon padding for better spacing
	const int iconPadding = H::Draw.Scale(12);
...
void CRadar::DrawBackground()
{
	...
	// Set colors
	const Color_t backgroundColor(0, 0, 0, 200);
	const Color_t accentColor = Vars::Menu::Theme::Accent.Value;
	const Color_t activeColor = Vars::Menu::Theme::Active.Value;

	// Draw main background with rounded corners
	const int cornerRadius = H::Draw.Scale(8);
	H::Draw.FillRoundRect(info.x, info.y, info.w, info.w, cornerRadius, backgroundColor);

	const int titleHeight = H::Draw.Scale(20);
	const int padding = H::Draw.Scale(12);
	const int radarAreaHeight = info.w - titleHeight - padding;
	const int radarAreaWidth = info.w - (padding * 2);

	// Removed cross drawing lines here
