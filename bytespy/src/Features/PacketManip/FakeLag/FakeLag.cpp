...
bool CFakeLag::IsAllowed(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	...
	// https://github.com/trfgokrteifgjr/cs/blob/0ccffa786cf5a513498b640a61eb9dda38d67f7e/source/client/features/misc/misc.cpp#L410
	float velocity_per_tick = pLocal->m_vecVelocity().Length2D() * I::GlobalVars->interval_per_tick;
	if (velocity_per_tick <= 0.f)
		velocity_per_tick = 1.f;

	int maxticks = Vars::CL_Move::Fakelag::PlainTicks.Value;
	int lag_comp_required = std::clamp((int)ceilf(64.f / velocity_per_tick), 0, 22);
	int min_ticks = Vars::CL_Move::Fakelag::FakelagEnum::BreakLC ? (lag_comp_required > maxticks ? maxticks : lag_comp_required) : maxticks;
	...
	case Vars::CL_Move::Fakelag::FakelagEnum::BreakLC:
		return I::ClientState->chokedcommands < min_ticks;
	}
...
void CFakeLag::Run(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd, bool* pSendPacket)
{
	...
	case Vars::CL_Move::Fakelag::FakelagEnum::BreakLC: m_iGoal = min_ticks;
	}
