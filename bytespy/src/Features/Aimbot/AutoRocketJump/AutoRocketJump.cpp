...
void CAutoRocketJump::ScoutJump(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
...
// idea from narrow 
void CAutoRocketJump::Detonator(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	if (!Vars::Misc::Movement::AutoRocketJump.Value) return;
	if (!pLocal || !pWeapon || !pCmd || !pLocal->IsAlive() || pLocal->IsAGhost()) return;
	if (pWeapon->m_iItemDefinitionIndex() != Pyro_s_TheDetonator) return;
	if (!G::CanPrimaryAttack) return;

	bool bGrounded = pLocal->OnSolid();
	//bool bDucking = pCmd->buttons & IN_DUCK;

	if (m_iFrame == -1 && bGrounded /*&& bDucking*/)
	{
		m_vAngles = pCmd->viewangles = { 89.f, pCmd->viewangles.y, 0.f };
		pCmd->buttons |= IN_DUCK | IN_JUMP | IN_ATTACK; 
		m_iFrame = 0;
		m_bDetonated = false;
		return;
	}
	...
	if (m_iFrame != -1)
	{
		...
		if (m_iFrame >= m_iDelay && m_iFrame <= m_iDelay + 2 && !m_bDetonated)
		{
			pCmd->buttons |= IN_ATTACK2;
			m_bDetonated = true;
		}

		if (m_iFrame > m_iDelay + 5)
		{
			m_iFrame = -1;
			m_bDetonated = false;
		}
