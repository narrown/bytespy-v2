...
{
	if (Vars::Aimbot::Projectile::ComputeTypeEnum::Normal)
	{
	...
	else if (Vars::Aimbot::Projectile::ComputeTypeEnum::FibonacciSequence)
	{
		std::vector<std::pair<Vec3, int>> vPoints;
		vPoints.reserve(iSamples);
		...
		{
			iPointType |= (PointTypeEnum::ObscuredMulti);
		}

		const float goldenAngle = PI * (3.f - sqrtf(5.f));

		for (int i = 0; i < iSamples; i++)
		{
			float y = 1.f - ((i + 0.5f) * (2.f / iSamples));
			float radiusAtY = sqrtf(1.f - y * y);
			float theta = goldenAngle * i;

			Vec3 vPoint = Vec3(cosf(theta) * radiusAtY,
				sinf(theta) * radiusAtY,
				y);

			vPoint = Math::RotatePoint(vPoint, {}, { flRotateX, flRotateY });

			if (flNthroot != 1.f) // wat?> 
			{
				vPoint.x = powf(fabsf(vPoint.x), 1 / flNthroot) * sign(vPoint.x);
				vPoint.y = powf(fabsf(vPoint.y), 1 / flNthroot) * sign(vPoint.y);
				vPoint.z = powf(fabsf(vPoint.z), 1 / flNthroot) * sign(vPoint.z);
	...
	// yes the entire idea of this is bad, you can still emulate a 4d splash sphere in a 3d space but you can't do it entirely, keyword emulate.
	if (Vars::Aimbot::Projectile::ComputeTypeEnum::Hypersphere)
	{
		std::vector<std::pair<Vec3, int>> vPoints;
		vPoints.reserve(iSamples);
		...
		for (int i = 0; i < iSamples; ++i)
		{
			...
			int baseType = PointTypeEnum::Regular;
			if (Vars::Aimbot::Projectile::SplashGrates.Value)
				baseType |= PointTypeEnum::Obscured;
			if (Vars::Aimbot::Projectile::RocketSplashMode.Value ==
				Vars::Aimbot::Projectile::RocketSplashModeEnum::SpecialHeavy)
			{
				baseType |= PointTypeEnum::ObscuredExtra | PointTypeEnum::ObscuredMulti;
			}

			float z = SDK::StdRandomFloat(-1.f, 1.f);
			float theta = SDK::StdRandomFloat(0.f, 2 * PI);
			float r = sqrtf(1.f - z * z);

			float x = r * cosf(theta);
			float y = r * sinf(theta);

			Vec3 projected = Vec3(x, y, z);
			projected = Math::RotatePoint(projected, {}, { flRotateX, flRotateY });
			projected *= flRadius;

			vPoints.emplace_back(projected, baseType);
		}
		return vPoints;
	}
	if (Vars::Aimbot::Projectile::ComputeTypeEnum::Optimized)
	{
		const int N = iSamples;
		std::vector<std::pair<Vec3, int>> vPoints;
		vPoints.reserve(N);

		const float goldenAngle = PI * (3.f - std::sqrt(5.f));
		const float invN = 1.f / static_cast<float>(N);

		float rotX = Vars::Aimbot::Projectile::SplashRotateX.Value;
		float rotY = Vars::Aimbot::Projectile::SplashRotateY.Value;
		if (rotX < 0.f) rotX = SDK::StdRandomFloat(0.f, 360.f);
		if (rotY < 0.f) rotY = SDK::StdRandomFloat(0.f, 360.f);
		...
		std::vector<std::pair<Vec3, int>> temp(N);
#pragma omp parallel for
		for (int i = 0; i < N; ++i)
		{
			// y goes from 1 to -1
			float y = 1.f - 2.f * i * invN;
			float radius = std::sqrt(std::max(0.f, 1.f - y * y));
			float theta = i * goldenAngle;

			// base point on unit sphere
			Vec3 p{ radius * std::cos(theta),
					radius * std::sin(theta),
				   -y };

			const float nth = Vars::Aimbot::Projectile::SplashNthRoot.Value;
			if (nth != 1.f)
			{
				auto stretch = [&](float coord) {
					float s = std::pow(std::fabs(coord), 1.f / nth);
					return s * (coord < 0 ? -1.f : 1.f);
					};
				p.x = stretch(p.x);
				p.y = stretch(p.y);
				p.z = stretch(p.z);
			...
			p *= Vars::Aimbot::Projectile::SplashRadius.Value;
			p = Math::RotatePoint(p, {}, { rotX, rotY });

			temp[i] = { p, baseType };
		}

		vPoints = std::move(temp);
		return vPoints;
	}
...
void CAimbotProjectile::CalculateAngle(const Vec3& vLocalPos, const Vec3& vTargetPos, Info_t& tInfo, int iSimTime, Solution_t& out, bool bAccuracy)
{
	...
	const float flGrav = tInfo.m_flGravity * 800.f;
	bool bHighArc = Vars::Aimbot::Projectile::HighArc.Value;

	if (Vars::Aimbot::Projectile::UnstableHighArc.Value)
	{
		const float flCycle = fmodf(I::GlobalVars->realtime, 0.6f); bHighArc = (flCycle >= 0.5f); // last 0.1s = high arc
	}
	else
	{
		bHighArc = Vars::Aimbot::Projectile::HighArc.Value && fmodf(I::GlobalVars->realtime, 0.2f) >= 0.1f;
	}
	...
	float flPitch, flYaw;
	{
		...
		else
		{
			...
			float flSqrtRoot = sqrt(flRoot);
			flPitch = atan((pow(flVelocity, 2) + (bHighArc ? flSqrtRoot : -flSqrtRoot)) / (flGrav * flDist));
		}
...

	{
		...
		else
		{
			...
			float flSqrtRoot = sqrt(flRoot);
			out.m_flPitch = atan((pow(flVelocity, 2) + (bHighArc ? flSqrtRoot : -flSqrtRoot)) / (flGrav * flDist));
		}
		out.m_flTime = flDist / (cos(out.m_flPitch) * flVelocity) + flDragTime;
	}

	{
		Vec3 vShootPos = (tProjInfo.m_vPos - vLocalPos).Get2D();
		Vec3 vTarget = vTargetPos - vLocalPos;
		Vec3 vForward; Math::AngleVectors(tProjInfo.m_vAng, &vForward); vForward.Normalize();
...
int CAimbotProjectile::CanHit(Target_t& tTarget, CTFPlayer* pLocal, CTFWeaponBase* pWeapon, std::deque<Vec3>* pPlayerPath, std::deque<Vec3>* pProjectilePath, std::vector<DrawBox_t>* pBoxes, float* pTimeTo)
{
	...
	if (Vars::Debug::ShowSplashPoints.Value)
	{
	...
	skipSplash:
		...
		for (auto& [vPoint, iPriority, iIndex] : vPoints) // get most ideal point
		{
			...
			bool bUseNeckbreaker = Vars::Aimbot::Projectile::RollAngles.Value &&
				Vars::Aimbot::General::AimType.Value == Vars::Aimbot::General::AimTypeEnum::Silent;

			if (bUseNeckbreaker)
			{
				int iStep = Vars::Aimbot::Projectile::RollAngleZ.Value;
				...
				{
					iRollsTried++;
					Vec3 vAngles; Aim(G::CurrentUserCmd->viewangles, { vPoint.m_Solution.m_flPitch, vPoint.m_Solution.m_flYaw, static_cast<float>(iRoll) }, vAngles);
			...
			{
				// Normal aiming without neckbreaker
