...
void CBinds::Run(CTFPlayer * pLocal, CTFWeaponBase * pWeapon)
{
	...
	std::function<void(int)> getBinds = [&](int iParent)
		{
			for (auto it = m_vBinds.rbegin(); it != m_vBinds.rend(); it++) // reverse order
			{
				...
				case BindEnum::Key:
				{
					...
					case BindEnum::KeyEnum::Hold:
						bKey = U::KeyHandler.Down(tBind.m_iKey, false, &m_mKeyStorage[tBind.m_iKey]);
						break;
					case BindEnum::KeyEnum::Toggle:
						bKey = U::KeyHandler.Pressed(tBind.m_iKey, false, &m_mKeyStorage[tBind.m_iKey]);
						break;
					case BindEnum::KeyEnum::DoubleClick:
						bKey = U::KeyHandler.Double(tBind.m_iKey, false, &m_mKeyStorage[tBind.m_iKey]);
						break;
					}

					const bool bShouldUse =
						(!I::EngineVGui->IsGameUIVisible() && (!I::MatSystemSurface->IsCursorVisible() || I::EngineClient->IsPlayingDemo())) ||
						(F::Menu.m_bIsOpen && !ImGui::GetIO().WantTextInput && !F::Menu.m_bInKeybind);
					...
					{
					case BindEnum::KeyEnum::Hold:
						tBind.m_bActive = tBind.m_bNot ? !bKey : bKey;
						break;
					case BindEnum::KeyEnum::Toggle:
					case BindEnum::KeyEnum::DoubleClick:
						if (bKey)
							tBind.m_bActive ^= true;
						break;
					}
					break;
				}
				case BindEnum::Class:
				{
					...
					tBind.m_bActive = (
						(tBind.m_iInfo == BindEnum::ClassEnum::Scout && iClass == 1) ||
						(tBind.m_iInfo == BindEnum::ClassEnum::Soldier && iClass == 3) ||
						(tBind.m_iInfo == BindEnum::ClassEnum::Pyro && iClass == 7) ||
						(tBind.m_iInfo == BindEnum::ClassEnum::Demoman && iClass == 4) ||
						(tBind.m_iInfo == BindEnum::ClassEnum::Heavy && iClass == 6) ||
						(tBind.m_iInfo == BindEnum::ClassEnum::Engineer && iClass == 9) ||
						(tBind.m_iInfo == BindEnum::ClassEnum::Medic && iClass == 5) ||
						(tBind.m_iInfo == BindEnum::ClassEnum::Sniper && iClass == 2) ||
						(tBind.m_iInfo == BindEnum::ClassEnum::Spy && iClass == 8)
						);
					if (tBind.m_bNot)
