...
#define InsertMultiCombo(x1,x2,x3,x4) ImGui::Spacing(); ImGui::NewLine(); ImGui::NewLine(); ImGui::SameLine(42.f); ImGui::PushItemWidth(158.f); ImGui::MultiCombo(x1, x2, x3, x4, x5, x6, x7, x8, x9); ImGui::PopItemWidth(); ImGui::CustomSpacing(1.f);
...
namespace ImGui
{
	...
	inline bool Section(const char* sLabel, bool bExtraPadding = false, float flMinHeight = 28.f, bool bForceHeight = false)
	{
		...
		const bool bReturn = ImGui::BeginChild(
			sLabel,
			ImVec2{ GetColumnWidth(), flMinHeight },
			false,
		...
		{
			if (sLabel[0] != '#')
				RenderTwoToneBackground(0.f, F::Render.Background0, F::Render.Background0p5, F::Render.Background2);
			else
				RenderBackground(0.f, F::Render.Background0p5, F::Render.Background2);
		}

		ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2{ H::Draw.Scale(8), 0 });
		if (sLabel[0] != '#')
		{
			const std::string stripped = StripDoubleHash(sLabel);
			ImVec2 textSz = ImGui::CalcTextSize(stripped.c_str());
			float headerH = H::Draw.Scale(bExtraPadding ? 27.f : 19.f);

			float availW = ImGui::GetContentRegionAvail().x;
			float posX = (availW - textSz.x) * 0.5f;
			float posY = (headerH - textSz.y) * 0.5f;

			ImGui::SetCursorPos(ImVec2{ posX, posY });
			ImGui::PushStyleColor(ImGuiCol_Text, F::Render.Accent.Value);
			ImGui::PushFont(F::Render.FontBold);
	...
	inline void EndSection()
	{
		...
		// dont know why we arent using ImGui::PopStyleVar(4); here but fuck me i guess
		ImGui::PopStyleVar(); 
		ImGui::EndChild();
		ImGui::PopStyleVar(); 
		ImGui::PopStyleVar(); 
	...
	inline bool FToggle(const char* sLabel, bool* pVar, int iFlags = FToggle_None, bool* pHovered = nullptr)
	{
		...
		if (iFlags & (FToggle_Left | FToggle_Right))
		{
			...
			float half = avail * 0.5f - style.WindowPadding.x * 1.5f;

			half = ImFloor(half + 0.5f);

			if (iFlags & FToggle_Right)
				ImGui::SameLine(half + style.WindowPadding.x * 2.0f);
		}

		bool changed = Disabled ? false : ImGui::Checkbox(sLabel, pVar);

		if (!Disabled && ImGui::IsItemHovered())
			ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);

		if (pHovered)
			*pHovered = ImGui::IsItemHovered();
	...
	inline bool FSlider(const char* sLabel, float* pVar1, float* pVar2, float flMin, float flMax, float flStep = 1.f, const char* fmt = "%.0f", int iFlags = FSlider_None, bool* pHovered = nullptr)
	{
		...
		static std::unordered_map<uint32_t, std::pair<float, float>> mAnimatedVars;
		if (!mActiveMap[uHash])
		...
		if (mAnimatedVars.find(uHash) == mAnimatedVars.end())
			mAnimatedVars[uHash] = { flSVar1, flSVar2 };
		float& flAVar1 = mAnimatedVars[uHash].first;
		float& flAVar2 = mAnimatedVars[uHash].second;

		// Lerp animated values towards static values
		const float flLerpSpeed = 0.2f;
		if (mActiveMap[uHash] || mActiveMap[uHash2]) {
			// Snap to actual value when interacting
			flAVar1 = flSVar1;
			flAVar2 = flSVar2;
		}
		else {
			// Smooth animation when not interacting
			flAVar1 += (flSVar1 - flAVar1) * flLerpSpeed;
			if (pVar2)
				flAVar2 += (flSVar2 - flAVar2) * flLerpSpeed;
		}

		// Reset animation if value changed externally
		if (flSVar1 != flOriginal1 || (pVar2 && flSVar2 != flOriginal2)) {
			flAVar1 = flSVar1;
			flAVar2 = flSVar2;
		}
		...
		if (pVar2)
		{
			float flLowerPerc = std::clamp((flAVar1 - flMin) / (flMax - flMin), 0.f, 1.f), flUpperPerc = std::clamp((flAVar2 - flMin) / (flMax - flMin), 0.f, 1.f);
			float flLowerPos = vMins.x + (vMaxs.x - vMins.x) * flLowerPerc, flUpperPos = vMins.x + (vMaxs.x - vMins.x) * flUpperPerc;

			// Modified AddSteppedRect calls to remove separators
			AddSteppedRect(vDrawPos, vMins, vMaxs, vMins, { flLowerPos, vMaxs.y }, flMin, flMax, flStep, vWashed, vWashed, H::Draw.Scale(2));
			AddSteppedRect(vDrawPos, vMins, vMaxs, { flLowerPos, vMins.y }, { flUpperPos, vMaxs.y }, flMin, flMax, flStep, vAccent, vAccent, H::Draw.Scale(2));
			AddSteppedRect(vDrawPos, vMins, vMaxs, { flUpperPos, vMins.y }, vMaxs, flMin, flMax, flStep, vWashed, vWashed, H::Draw.Scale(2));
		...
		{
			float flPercent = std::clamp((flAVar1 - flMin) / (flMax - flMin), 0.f, 1.f);
			float flPos = vMins.x + (vMaxs.x - vMins.x) * flPercent;

			// Modified AddSteppedRect calls to remove separators
			AddSteppedRect(vDrawPos, vMins, vMaxs, vMins, { flPos, vMaxs.y }, flMin, flMax, flStep, vAccent, vAccent, H::Draw.Scale(2));
			AddSteppedRect(vDrawPos, vMins, vMaxs, { flPos, vMins.y }, vMaxs, flMin, flMax, flStep, vWashed, vWashed, H::Draw.Scale(2));
	...
	// The rest of the overloaded FSlider functions remain unchanged
	...
	inline bool FDropdown(const char* sLabel, int* pVar, std::vector<const char*> vEntries, std::vector<int> vValues = {}, int iFlags = FDropdown_None, int iSizeOffset = 0, bool* pHovered = nullptr, int* pModified = nullptr)
	{
		...
		PushStyleVar(ImGuiStyleVar_PopupRounding, 0.0f);

		ImVec4 darkGrey = ImVec4(12.0f / 255.0f, 12.0f / 255.0f, 12.0f / 255.0f, 1.0f);
		PushStyleColor(ImGuiCol_FrameBg, darkGrey);
		PushStyleColor(ImGuiCol_PopupBg, darkGrey);
		...
		// Reduced base height
		ImVec2 vSize = { GetWindowWidth(), H::Draw.Scale(bTitle ? 30 : 20) };
		...
		PushStyleVar(ImGuiStyleVar_FramePadding, { 0.f, H::Draw.Scale(bTitle ? 8.0f : 4.0f) });
		...
		if (bActive)
		{
			...
			PushStyleVar(ImGuiStyleVar_ItemSpacing, { 0, H::Draw.Scale(12) });
			int i = 0;
			for (auto& sEntry : vEntries)
			{
				if (FNV1A::Hash32(sEntry) == FNV1A::Hash32Const("##Divider"))
				{
					...
					ImColor sepColor = ImColor(darkGrey);
					sepColor.Value.w *= GetStyle().Alpha;
					GetWindowDrawList()->AddRectFilled(
						{ vDrawPos.x + H::Draw.Scale(18), vDrawPos.y + flPosY },
						{ vDrawPos.x + GetWindowWidth() - H::Draw.Scale(18), vDrawPos.y + flPosY + 1 },
						sepColor
					);
					DebugDummy({});
					continue;
				...
				if (iFlags & FDropdown_Multi)
				{
					...
					TextUnformatted(sStripped.c_str());
					PopStyleColor();
					SetCursorPos({ vOriginalPos2.x + H::Draw.Scale(16), vOriginalPos2.y });
				...
				else
				{
					...
					ImVec2 vOriginalPos3 = GetCursorPos();
					SetCursorPos({ vOriginalPos2.x + H::Draw.Scale(iFlags & FDropdown_Modifiable ? 40 : 20), vOriginalPos2.y });
		...
		// ===== CRITICAL FIX =====
		// Handle hover state reporting for bind popups
		if (!Disabled && IsItemHovered())
			SetMouseCursor(ImGuiMouseCursor_Hand);
		if (pHovered)  // <-- Added hover detection
			*pHovered = ImGui::IsItemHovered();
	...
	inline bool FSDropdown(const char* sLabel, std::string* pVar, std::vector<const char*> vEntries = {}, int iFlags = FDropdown_None, int iSizeOffset = 0, bool* pHovered = nullptr)
	{
		...
		// → force dark grey (12,12,12) backgrounds for filesystem dropdown
		ImVec4 darkGreyFS = ImVec4(12.0f / 255.0f, 12.0f / 255.0f, 12.0f / 255.0f, 1.0f);
		PushStyleColor(ImGuiCol_FrameBg, darkGreyFS);
		PushStyleColor(ImGuiCol_PopupBg, darkGreyFS);
		...
		if (BeginCombo(std::format("##{}", sLabel).c_str(), "", ImGuiComboFlags_CustomPreview | ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_HeightLarge))
		{
			...
			ImVec2 vOriginalPos2 = GetCursorPos();
			SetCursorPos({ -1000, vEntries.empty() ? -100 : GetScrollY() });
			if (!IsAnyItemActive())
				SetKeyboardFocusHere();
				...
				for (int i = 0; i < vValid.size(); i++)
				{
					...
					if (FNV1A::Hash32(sEntry.c_str()) == FNV1A::Hash32Const("##Divider"))
					{
						if (!bDivider)
						{
							ImVec2 vDrawPos = GetDrawPos();
							float flPosY = GetCursorPosY();
							ImColor vInactive = F::Render.Inactive;
							vInactive.Value.w *= GetStyle().Alpha;
							GetWindowDrawList()->AddRectFilled(
								{ vDrawPos.x + H::Draw.Scale(18), vDrawPos.y + flPosY },
								{ vDrawPos.x + GetWindowWidth() - H::Draw.Scale(18), vDrawPos.y + flPosY + 1 },
								vInactive
			...
			if (bTitle)
			{
				...
				if (mActiveMap[uHash] || iFlags & FSDropdown_Custom || vEntries.empty())
				{
					...
					GetWindowDrawList()->AddRectFilled(
						...
						{ vDrawPos.x + vSize.x - H::Draw.Scale(vEntries.empty() ? 25 : 45), vDrawPos.y + 1 },
	...
	*/
	...
	inline bool FColorPicker(const char* sLabel, Color_t* tColor, int iOffset = 0, int iFlags = FColorPicker_None, bool* pHovered = nullptr)
	{
		...
		// —––––––— unify preview size to ImGui’s standard widget height —––––––—
		const ImGuiStyle& style = GetStyle();
		float               square_sz = GetFrameHeight(); // framePadding*2 + font size

		ImVec2 vOriginalPos;
		ImVec2 vSize = { square_sz, square_sz };
		...
		if (!(iFlags & FColorPicker_Dropdown))
		{
			...
			else
			{
				...
				if (iFlags & FColorPicker_SameLine)
					SameLine();
				SetCursorPosX(GetContentRegionMax().x - (square_sz + iOffset * square_sz));

				vOriginalPos = GetCursorPos();

				SetCursorPosY(vOriginalPos.y + style.FramePadding.y);
				bReturn = ColorPicker(sLabel, tColor, true);

				SetCursorPos(vOriginalPos);
				DebugDummy({});
				vOriginalPos.y += style.FramePadding.y;
			}
		}
		else
		{
			...
			SameLine();
			vSize = { square_sz * 0.833f, flOriginalPosY - GetCursorPosY() - style.WindowPadding.y };

			SetCursorPosX(GetCursorPosX() - style.WindowPadding.y);
			vOriginalPos = GetCursorPos();
			...
			SetCursorPos(vOriginalPos);
			AddRowSize(vOriginalPos.y, vSize.y + style.WindowPadding.y);
			DebugDummy({ vSize.x, GetRowSize(vSize.y + style.WindowPadding.y) });

			vOriginalPos.y += style.WindowPadding.y;
			vSize = { square_sz * 0.833f, square_sz * 3.333f };
		}

		// hover hit-test uses the exact same vSize now
		if (pHovered && IsWindowHovered())
		{
			vOriginalPos += GetDrawPos();
			*pHovered = IsMouseWithin(vOriginalPos.x, vOriginalPos.y, vSize.x, vSize.y);
	...
	inline bool FMDropdown(const char* sLabel, std::vector<std::pair<std::string, Color_t>>* pVar, int iFlags = FDropdown_None, int iSizeOffset = 0, bool* pHovered = nullptr)
	{
		...
		std::sort(vMaterials.begin(), vMaterials.end(), [&](const auto& a, const auto& b) -> bool
			{
				...
				if (a.m_bLocked && !b.m_bLocked) return true;
				if (!a.m_bLocked && b.m_bLocked) return false;
		...
		// → force dark grey (12,12,12) backgrounds for material dropdown
		ImVec4 darkGrey = ImVec4(12.0f / 255.0f, 12.0f / 255.0f, 12.0f / 255.0f, 1.0f);
		PushStyleColor(ImGuiCol_FrameBg, darkGrey);
		PushStyleColor(ImGuiCol_PopupBg, darkGrey);
		...
		else
		{
			...
			sPreview.resize(sPreview.size() - 2);
		}
		...
		// — match FDropdown’s reduced vertical padding:
		PushStyleVar(ImGuiStyleVar_FramePadding, { 0.f, H::Draw.Scale(bTitle ? 8.0f : 4.0f) });
		PushItemWidth(vSize.x);
		...
		if (BeginCombo(std::format("##{}", sLabel).c_str(), "", ImGuiComboFlags_CustomPreview | ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_HeightLarge))
		{
			...
			// — match FDropdown’s top dummy:
			DebugDummy({ 0, H::Draw.Scale(4) });
			// — match FDropdown’s item spacing:
			PushStyleVar(ImGuiStyleVar_ItemSpacing, { 0, H::Draw.Scale(12) });
			...
			for (int i = 0; i < vEntries.size(); i++)
			{
				...
				if (FNV1A::Hash32(sEntry.c_str()) == FNV1A::Hash32Const("##Divider"))
				{
					ImVec2 dp = GetDrawPos();
					float y = GetCursorPosY();
					ImColor sep = F::Render.Inactive; sep.Value.w *= GetStyle().Alpha;
					GetWindowDrawList()->AddRectFilled(
						{ dp.x + H::Draw.Scale(18), dp.y + y },
						{ dp.x + GetWindowWidth() - H::Draw.Scale(18), dp.y + y + H::Draw.Scale(1) },
						sep
					);
					DebugDummy({});
					continue;
				...
				auto itFind = mIts.find(sEntry);
				bool bFlagActive = itFind != mIts.end();
				int iEntry = bFlagActive ? std::distance(pVar->begin(), itFind->second) + 1 : 0;

				ImVec2 pos0 = GetCursorPos();
				if (bFlagActive)
				{
					SetCursorPos({ pos0.x + vSize.x - H::Draw.Scale(31), pos0.y + H::Draw.Scale(1) });
					ColorPicker(std::format("MaterialColor{}", iEntry).c_str(), &itFind->second->second, false);
					SetCursorPos(pos0);
				}
				bool hoveredBefore = bFlagActive ? IsItemHovered() : false;

				if (FSelectable(std::format("##{}{}", sEntry, i).c_str(), nullptr, 0, bFlagActive, ImGuiSelectableFlags_DontClosePopups))
				{
					if (bFlagActive)      pVar->erase(itFind->second);
					else                  pVar->emplace_back(sEntry, Color_t());
					bReturn = true;
				}
				bool hovered = !hoveredBefore && IsItemHovered();

				ImVec2 pos2 = GetCursorPos();
				SetCursorPos({ pos0.x + H::Draw.Scale(40 + 6 * int(log10(std::max(pVar->size(),1ui64)))), pos0.y + H::Draw.Scale(1) });
				PushStyleColor(ImGuiCol_Text, bFlagActive ? F::Render.Active.Value : F::Render.Inactive.Value);
				TextUnformatted(hovered ? sEntry.c_str() : TruncateText(sEntry, vSize.x - (bFlagActive ? 82 : 60)).c_str());
				PopStyleColor();

				if (bFlagActive)
				{
					SetCursorPos({ pos0.x + H::Draw.Scale(18), pos0.y + H::Draw.Scale(1) });
					PushStyleColor(ImGuiCol_Text, F::Render.Accent.Value);
					Text("%i", iEntry);
					PopStyleColor();
				}
			...
			// — match FDropdown’s bottom dummy:
			SetCursorPosY(GetCursorPosY() - H::Draw.Scale(5));
			DebugDummy({});
			EndCombo();
		}
		...
		if (BeginComboPreview())
		{
			ImVec2 pp = GetCursorPos();
			if (bTitle)
			{
				SetCursorPos({ pp.x + H::Draw.Scale(12), pp.y - H::Draw.Scale(5) });
				PushFont(F::Render.FontSmall);
				PushStyleColor(ImGuiCol_Text, F::Render.Inactive.Value);
				TextUnformatted(TruncateText(StripDoubleHash(sLabel), vSize.x - H::Draw.Scale(45)).c_str());
				PopStyleColor();
				...
				SetCursorPos({ pp.x + H::Draw.Scale(12), pp.y + H::Draw.Scale(8) });
				TextUnformatted(TruncateText(sPreview, vSize.x - H::Draw.Scale(45)).c_str());

				SetCursorPos({ pp.x + vSize.x - H::Draw.Scale(24), pp.y - H::Draw.Scale(1) });
				IconImage(bActive ? ICON_MD_KEYBOARD_ARROW_UP : ICON_MD_KEYBOARD_ARROW_DOWN);
			}
			else
			{
				SetCursorPos({ pp.x + H::Draw.Scale(12), pp.y });
				TextUnformatted(TruncateText(sPreview, vSize.x - H::Draw.Scale(45)).c_str());
				SetCursorPos({ pp.x + vSize.x - H::Draw.Scale(24), pp.y - H::Draw.Scale(1) });
				IconImage(bActive ? ICON_MD_KEYBOARD_ARROW_UP : ICON_MD_KEYBOARD_ARROW_DOWN);
			}
		...
		// cleanup our pushed colors
		PopStyleColor(2);

		if (pHovered && IsWindowHovered())
		{
			ImVec2 dp = GetDrawPos() + vOriginalPos;
			*pHovered = IsMouseWithin(dp.x, dp.y + GetStyle().WindowPadding.y, vSize.x, vSize.y);
		}
	...
	inline void DrawBindInfo(ConfigVar<T>& var, T& val, std::string sBind, bool bNewPopup, bool& bLastHovered)
	{
		...
		{
		case BindEnum::Key: tBind.m_iInfo = std::clamp(tBind.m_iInfo, 0, 2); FDropdown("behavior", &tBind.m_iInfo, { "held", "toggled", "double click" }, {}, FDropdown_Right, 0, &bHovered); break;
