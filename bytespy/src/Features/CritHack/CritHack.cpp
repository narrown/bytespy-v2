...
void CCritHack::CanFireCritical(CTFPlayer* pLocal, CTFWeaponBase* pWeapon)
{
	...
	{
		m_iResourceDamage = static_cast<int>(reinterpret_cast<uintptr_t>(pResource->m_iDamage())); // meh
...
void CCritHack::Draw(CTFPlayer* pLocal)
{
	...
	bool bCanThisWeaponCrit = WeaponCanCrit(pWeapon, true);

	ConVar* crits = I::CVar->FindVar("tf_weapon_criticals");
	if (crits && crits->GetInt() == 0)
	{
		bCanThisWeaponCrit = false;
	}

	if (!bCanThisWeaponCrit)
	{
		...
		H::Draw.StringOutlined(
			...
			iPosY + iPadding,
			{
				static_cast<byte>(150 + (((std::sin(I::GlobalVars->curtime * 5.0f) * 0.5f) + 0.5f) * (255 - 150))),
				static_cast<byte>(0 + (((std::sin(I::GlobalVars->curtime * 5.0f) * 0.5f) + 0.5f) * (80 - 0))),
				static_cast<byte>(0 + (((std::sin(I::GlobalVars->curtime * 5.0f) * 0.5f) + 0.5f) * (80 - 0))),
				255
			},
			{ 0, 0, 0, 150 },
	...
	const int   iAvailable = tStorage.m_iAvailableCrits;
	const int   iPotential = tStorage.m_iPotentialCrits;
	float flTargetRatio = (iPotential > 0)
		? static_cast<float>(iAvailable) / iPotential
		: 0.0f;

	static float flDisplayedCritRatio = 0.0f;
	constexpr float SMOOTH_SPEED = 15.0f;
	float delta = I::GlobalVars->frametime * SMOOTH_SPEED;
	delta = std::clamp(delta, 0.0f, 1.0f);
	flDisplayedCritRatio += (flTargetRatio - flDisplayedCritRatio) * delta;
	...
	H::Draw.StringOutlined(
		...
		{ 0, 0, 0, 150 },
		ALIGN_TOPLEFT,
		std::format("crits {}/{}", iAvailable, iPotential).c_str()
	);

	std::string sStatus;
	Color_t clrStatus = Vars::Menu::Theme::Active.Value;
	auto flTickBase = TICKS_TO_TIME(pLocal->m_nTickBase());
	...
	if (pLocal->IsCritBoosted()) {
		sStatus = "boosted";
		clrStatus = Vars::Colors::IndicatorTextMisc.Value;
	}
	else if (pWeapon->m_flCritTime() > flTickBase) {
		sStatus = std::format("{:.1f}s", pWeapon->m_flCritTime() - flTickBase);
		clrStatus = Vars::Colors::IndicatorTextMisc.Value;
	}
	else if (m_bCritBanned && iSlot != SLOT_MELEE) {
		sStatus = std::format("deal {}", ceilf(m_flDamageTilFlip));
		clrStatus = Vars::Colors::IndicatorTextBad.Value;
	}
	else if (tStorage.m_iAvailableCrits > 0) {
		sStatus = "ready";
		clrStatus = Vars::Menu::Theme::Accent.Value;
	}
	else {
		sStatus = std::format("{} shot{}", tStorage.m_iNextCrit, tStorage.m_iNextCrit == 1 ? "" : "s");
		clrStatus = Vars::Colors::IndicatorTextBad.Value;
	}
	...
	const int iLineY = iPosY + iHeight - iLineHeight - iPadding;
	const int iLineWidth = iWidth - iPadding * 2;
	const int iFilledWidth = static_cast<int>(iLineWidth * flDisplayedCritRatio);
