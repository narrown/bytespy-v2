...
void COutput::OutputInfo(int flags, std::string sName, std::string sOutput, std::string sChat)
{
	...
	if (flags & Vars::Logging::LogToEnum::Chat)
		SDK::Output(Vars::Menu::CheatPrefix.Value.c_str(), sChat.c_str(), Vars::Menu::Theme::Accent.Value, false, false, false, false, true, false, -1, "", " "); // grizz you like penis
}
...
void COutput::Event(IGameEvent* pEvent, uint32_t uHash, CTFPlayer* pLocal)
{
	...
	case FNV1A::Hash32Const("vote_cast"): // Voting
	{
		...
		OutputInfo(Vars::Logging::VoteCast::LogTo.Value, " ", sOutput, sChat);
	...
	case FNV1A::Hash32Const("player_changeclass"): // Class change
	{
		...
		OutputInfo(Vars::Logging::ClassChange::LogTo.Value, " ", sOutput, sChat);
	...
	case FNV1A::Hash32Const("player_hurt"): // Damage
	{
		...
		auto sName = F::PlayerUtils.GetPlayerName(iIndex, pi.name);
		std::string sOutput = std::format("hit {} for {} damage{}", (sName), (nDamage), (bCrit ? " (crit) " : " "), (nHealth), (iMaxHealth));
		std::string sChat = std::format(
			"hit {}{}\x1 for {}{} damage{}",
			yellow,                // #1
			sName,                 // #2
			red,                   // #3
		...
		OutputInfo(Vars::Logging::Damage::LogTo.Value, " ", sOutput, sChat);
...
void COutput::UserMessage(bf_read& msgData)
{
	...
	std::string sOutput = std::format("{}{} initiated a vote on {}", (bSameTeam ? "" : "[enemy] "), (sCallerName), (sTargetName));
	std::string sChat = std::format("{}{}{}\x1 initiated a vote on {}{}", (bSameTeam ? "" : "[enemy] "), (yellow), (sCallerName), (yellow), (sTargetName));
	OutputInfo(Vars::Logging::VoteStart::LogTo.Value, " ", sOutput, sChat);
}
...
void COutput::CheatDetection(std::string sName, std::string sAction, std::string sReason)
{
	...
	OutputInfo(Vars::Logging::CheatDetection::LogTo.Value, " ", sOutput, sChat);
}
...
void COutput::TagsOnJoin(std::string sName, uint32_t friendsID)
{
	...
	OutputInfo(Vars::Logging::Tags::LogTo.Value, " ", sOutput, sChat);
}
...
void COutput::ReportResolver(std::string sMessage)
{
	...
	OutputInfo(Vars::Logging::Tags::LogTo.Value, " ", sMessage, sMessage);
}
void COutput::ReportVaccinator(int iIndex, std::string sAction, std::string sResist, std::string sReason)
{
	if (!(Vars::Logging::Logs.Value & Vars::Logging::LogsEnum::Vaccinator))
		return;
	...
	std::string sOutput = std::format("{} {} resist for {} due to {}", sAction, sResist, sName, sReason);
	std::string sChat = std::format("{}{} {}{}\x1 resist for {}{}\x1 due to {}{}\x1",
		green, white, yellow, sResist, yellow, sName, yellow, sReason);

	OutputInfo(Vars::Logging::Vaccinator::LogTo.Value, "[bytespy]", sOutput, sChat);
}
