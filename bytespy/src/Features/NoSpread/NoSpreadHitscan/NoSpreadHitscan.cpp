...
void CNoSpreadHitscan::Draw(CTFPlayer* pLocal)
{
	if (!(Vars::Menu::Indicators.Value & Vars::Menu::IndicatorsEnum::SeedPrediction)
		...
		|| !Vars::Aimbot::General::NoSpread.Value)
		return;
	...
	// Conditional rounded background
	...
	const float flUptime = m_flServerTime;
	const float flMantissaStep = m_flMantissaStep;

	H::Draw.StringOutlined(
		fFont, iPosX + iPadding, iPosY + iPadding,
		cColor, { 0, 0, 0, 150 }, ALIGN_TOPLEFT,
		std::format("time {}", GetFormat(flUptime)).c_str()
	);

	H::Draw.StringOutlined(
		fFont, iPosX + iWidth - iPadding, iPosY + iPadding,
		cColor, { 0, 0, 0, 150 }, ALIGN_TOPRIGHT,
		std::format("step {}", flMantissaStep).c_str()
	);

	// compute raw cycle ratio [0..1)
	const float flCycle = std::fmod(flUptime, 1.0f);

	// Smooth interpolation
	static float flDisplayedCycle = 0.0f;
	constexpr float SMOOTH_SPEED = 15.0f;
	float delta = I::GlobalVars->frametime * SMOOTH_SPEED;
	delta = std::clamp(delta, 0.0f, 1.0f);
	flDisplayedCycle += (flCycle - flDisplayedCycle) * delta;

	// Bar drawing
	const int iBarY = iPosY + iHeight - iLineHeight - iPadding;
	const int iBarWidth = static_cast<int>((iWidth - iPadding * 2) * flDisplayedCycle);

	// Always draw lighter inner bar background
	...
	{
		H::Draw.StringOutlined(
			fFont, iPosX + iWidth / 2, iBarY + iLineHeight + iPadding,
			cColor, { 0, 0, 0, 150 }, ALIGN_TOP,
			std::format("delta {:.6f}", m_dTimeDelta).c_str()
		);
	}
}
