...
namespace Vars
{
	NAMESPACE_BEGIN(Menu)
		CVar(CheatName, std::string("bytespy"), VISUAL)
		CVar(CheatPrefix, std::string("[bytespy]"), VISUAL)
		...
		CVar(RemoveBackground, false)

		CVar(Watermark, true)

		CVarEnum(Indicators, 0b00000, NONE, Ticks = 1 << 0, CritHack = 1 << 1, Spectators = 1 << 2, Ping = 1 << 3, SeedPrediction = 1 << 4, Watermark = 1 << 5, Spotify = 1 << 6, MovementRecorder = 1 << 7, Conditions = 1 << 8) // make sure if you add a new indicator you push conditions back because we don't use this shit!
		//CVar(SpectatorAvatars, false, VISUAL)

		CVar(TicksDisplay, DragBox_t(), NOBIND)
		CVar(SpotifyDisplay, DragBox_t(), NOBIND)
		...
		CVar(MovementRecorderDisplay, DragBox_t(), NOBIND)

		CVar(Scale, 1.f, NOBIND)
		CVar(CheapText, false)

		SUBNAMESPACE_BEGIN(Theme)
		CVar(Accent, Color_t(255, 50, 113, 255), VISUAL)
		CVar(Conditions, Color_t(0, 255, 255, 255), VISUAL)
		...
		NAMESPACE_BEGIN(MovementRecorder)
		CVar(Enabled, false)
		CVar(Playback, false)
		CVar(Save, false)

		CVar(Line, false)
		CVar(LineColor, Color_t(255, 255, 255, 255), VISUAL)

		CVar(VisualizeSpot, false)
		CVar(VisualizeSpotColor, Color_t(255, 255, 255, 255), VISUAL)
	NAMESPACE_END(MovementRecorder)
	...
	NAMESPACE_BEGIN(Aimbot)
		...
		SUBNAMESPACE_BEGIN(Hitscan)
			...
			CVarEnum(StaticHitboxes, 0b000111, NONE, Head = 1 << 0, Body = 1 << 1, Pelvis = 1 << 2, Arms = 1 << 3, Legs = 1 << 4)
		...
		SUBNAMESPACE_BEGIN(Projectile)
			...
			CVarEnum(AutoAirblast, 0b1000, NONE, Enabled = 1 << 0, RedirectSimple = 1 << 1, RedirectAdvanced = 1 << 2, RespectFOV = 1 << 3, AutoAirblastIgnoreDG = 1 << 4)
			...
			CVar(HighArc, false)
			CVar(UnstableHighArc, false)
			CVar(RollAngles, false)
			CVar(RollAngleZ, 20.f)

			CVarEnum(ComputeType, 0b10001, NONE, Normal = 1 << 0, Optimized = 1 << 1, FibonacciSequence = 1 << 2, Hypersphere = 1 << 3)
			...
			CVar(SplashPoints, 100, NONE)
			CVar(SplashGrates, true, DEBUGVAR)
			CVar(SplashRotateX, 0.f, NONE)
			CVar(SplashRotateY, 0.f, NONE)
			...
			CVarEnum(RocketSplashMode, 0, NONE, Regular, SpecialLight, SpecialHeavy)
			CVar(DeltaCount, 5, DEBUGVAR)
			CVarEnum(DeltaMode, 0, NONE, Average, Max)
			CVarEnum(MovesimFrictionFlags, 0, NONE, RunReduce, CalculateIncrease)
	...
	NAMESPACE_BEGIN(CL_Move)
		SUBNAMESPACE_BEGIN(Doubletap)
			...
			CVar(NoDTinair, false)
			CVar(OnlyDTatlethal, false)
			CVar(AntiWarp, true)
			CVar(TickLimit, 22)
			CVar(WarpRate, 22)
		...
		SUBNAMESPACE_BEGIN(Fakelag)
			CVarEnum(Fakelag, 0, NONE, Off, Plain, Random, Adaptive, BreakLC)
			CVar(PlainTicks, 12)
		...
		CVar(FakeDuck, false)
		CVar(FakeDuckTicks, 1)

	    CVar(AutoPeek, false)
		CVar(AutoPeekColor, Color_t(255, 255, 255, 0), VISUAL)
	...
	NAMESPACE_BEGIN(ESP)
		...
		CVar(CustomHPColor, false, VISUAL)
		CVar(HPColor, Color_t(255, 255, 255, 255), VISUAL)
	NAMESPACE_END(ESP)
	...
	NAMESPACE_BEGIN(Visuals)
		...
		SUBNAMESPACE_BEGIN(UI)
			...
			CVar(NetGraphWatermark, true, VISUAL)
		SUBNAMESPACE_END(Viewmodel)
		...
		SUBNAMESPACE_BEGIN(Simulation)
			Enum(Style, Off, Line, Separators, Spaced, Arrows, Boxes, Outlined);
		...
		SUBNAMESPACE_BEGIN(Freecam)
			CVar(Enabled, false, VISUAL)
			CVar(Speed, 0.f, VISUAL)
		SUBNAMESPACE_END(ThirdPerson)

		SUBNAMESPACE_BEGIN(SkinChanger)
				CVar(Enabled, false, VISUAL)
				CVar(Festivized, false, VISUAL)
				CVar(Australium, false, VISUAL)
				CVarEnum(Unusual, 0b00000, VISUAL, None = 0, Cool = 1 << 0, Hot = 1 << 1, Isotope = 1 << 2, Energy_Orb = 1 << 3)
		...
		SUBNAMESPACE_BEGIN(FOVArrows)
			...
			CVarEnum(OOBStyle, 0, VISUAL, Diamond = 0, Arrow = 1)
		...
		SUBNAMESPACE_BEGIN(Misc)
			...
			CVar(ShowBindMenu, true)
		SUBNAMESPACE_END(Misc)

		SUBNAMESPACE_BEGIN(Killfeed)
				CVarEnum(Icon, 0, VISUAL,None = 0,Sawblade = 1 << 0,Pumpkin = 1 << 1, Armageddon = 1 << 2, Trains = 1 << 3, Skull = 1 << 4, Saxxy = 1 << 5)
				CVarEnum(Overlays, 0, VISUAL,None = 0,Australium = 1 << 0,Crit = 1 << 1)
				CVarEnum(Domination, 0b00000, VISUAL, None = 1 << 0, Domination = 1 << 1, Revenge = 1 << 2, Both = 1 << 3)
		SUBNAMESPACE_END(Killfeed)
	...
	NAMESPACE_BEGIN(Radar)
		SUBNAMESPACE_BEGIN(Main)
			...
			CVarEnum(Style, 0, VISUAL, Rectangle)
	...
	NAMESPACE_BEGIN(Misc)
		SUBNAMESPACE_BEGIN(Movement)
			...
			CVar(BlockBot, false)
			CVar(BlockBotSnipersOnly, false)
			CVar(BlockBotOffset, 25.0f)
			CVar(Bunnyhop, false)
			//CVar(AutoDetonatorJump, false)
			CVar(EdgeJump, false)
			...
			CVar(NavBot, false)
			CVar(NavBotAvoidWalls, false)
			CVar(NavbotDraw, false)
			CVar(NavbotDrawColor, Color_t(255, 50, 113, 255), VISUAL)
		...
		SUBNAMESPACE_BEGIN(Automation)
			...
			CVar(AutoVaccinator_Active, false)
			CVarEnum(Vaccinatortargets, 0, NONE, Everyone, Friends)
		SUBNAMESPACE_END(Automation)

		SUBNAMESPACE_BEGIN(Sound)
			CVarEnum(Block, 0b0000, NONE, Footsteps = 1 << 0, Noisemaker = 1 << 1, FryingPan = 1 << 2, Water = 1 << 3, Door = 1 << 4)
		...
		SUBNAMESPACE_BEGIN(Steam)
			...
			CVar(MapText, std::string("bytespy"))
			CVar(GroupSize, 1337)
		SUBNAMESPACE_END(Steam)
	NAMESPACE_END(Misc)

	NAMESPACE_BEGIN(Logging)
		CVarEnum(Logs, 0b0000011, NONE, VoteStart = 1 << 0, VoteCast = 1 << 1, ClassChanges = 1 << 2, Damage = 1 << 3, CheatDetection = 1 << 4, Tags = 1 << 5, Aliases = 1 << 6, Resolver = 1 << 7, Vaccinator = 1 << 8)
		...
		SUBNAMESPACE_END(Resolver)
			...
			SUBNAMESPACE_BEGIN(Vaccinator)
				CVar(LogTo, 0b000001)
			SUBNAMESPACE_END(Vaccinator)
	...
	NAMESPACE_BEGIN(Debug)
		...
		CVar(ShowSplashPoints, false, NOSAVE)
