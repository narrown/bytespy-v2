...
const static std::vector<const char*> vDoorOpen = { "doors\\door_metal_rusty_move1"}; // maybe add generic_door_close and generic_door_open?
...
static bool ShouldBlockSound(const char* pSound)
{
	...
	if (Vars::Misc::Sound::Block.Value)
	{
		...
		if (CheckSound(Vars::Misc::Sound::BlockEnum::Door, vDoorOpen))
			return true;
	}
