...
#include "../Features/Misc/MovementRecorder/MovementRecorder.h"
	...
	CClientModeShared* rcx, float flInputSampleTime, CUserCmd* pCmd)
{
	...
	F::MovementRecorder.CreateMove(pCmd);
	...
	F::Resolver.CreateMove(pLocal);	G::Choking = !*pSendPacket;
	...
	if (pLocal)
	{
		static std::vector<Vec3> vChokedAngles;
		vChokedAngles.push_back(pCmd->viewangles);
		...
		if (pAnimState)
		{
			...
			if (*pSendPacket)
			{
				for (size_t i = 0; i < vChokedAngles.size(); i++)
				{
					Vec3 animAngle = vChokedAngles[i];

					if (i == vChokedAngles.size() - 1 &&
						Vars::AntiHack::AntiAim::Enabled.Value)
					{
						animAngle.y = F::AntiAim.GetYaw(pLocal, pCmd, true);
						animAngle.x = F::AntiAim.GetPitch(animAngle.x);
					}

					if (pLocal->IsTaunting() && pLocal->m_bAllowMoveDuringTaunt())
						pLocal->m_flTauntYaw() = animAngle.y;

					pAnimState->m_flEyeYaw = animAngle.y;
					pAnimState->Update(animAngle.y, animAngle.x);
